              ...
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4" :key="name">
                    <v-text-field v-model="editedItem[name]" :label="getLabelName(name)"></v-text-field>
                  </v-col>
                  </template>
                </v-row>
              </v-container>
...
export default {
  props: ["headers", "items", "crudObject", "title", "crudUrl"],
  data: () => ({
      dialog: false,
      editedIndex: -1,
      editedItem: {},
    ...
    methods: {
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          rentalApi.post(this.crudUrl, this.editedItem)
            .then(response => {
